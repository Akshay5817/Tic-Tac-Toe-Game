...
$(document).ready(function()
{
	...
	var turn = "X";
	var gameMode = true		// checks whether the play has stopped
	...
	$("td").click(function()
	{
		if (gameMode)
		{
			if($(this).text() == "")
			{
				...
				count += 1;
				if(count == 9)							// checks if no more place is empty
				{
					$("#comments h3").text("");
				...
				turn = change(turn);		// changes the turn X to 0 or 0 to X				
...
// function to change the value of X to 0 or 0 to X
...
// checks the logic for winning the game
...
function wins(turn)
{
	...
	$("#divrestart").slideDown("1000");				// displays restart button
}